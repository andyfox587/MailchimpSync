...
router.post('/select-audience', express.urlencoded({ extended: true }), async (req, res) => {
  try {
    ...
    // Try auto-mapping: look up restaurant in vivaspot_sites by account name
    const site = await db.findSiteByRestaurantName(metadata.accountName);
    
    if (site && site.mac_addresses && site.mac_addresses.length > 0) {
      // Auto-map all MAC addresses from the site
      console.log(`Auto-mapping ${site.mac_addresses.length} MAC(s) for "${site.restaurant_name}"`);
      ...
      // No auto-mapping match - redirect to manual setup
...
function renderSuccessPage(res, accountName, audienceName, redirectUrl, deviceCount = 1) {
